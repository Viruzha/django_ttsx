...
class user(models.Model):
    '''用户信息类'''
    usessionid = models.CharField(
        '用户登录id', max_length=100)
    # 用户头像图片
    uhead = models.ImageField(
        u'用户头像', upload_to='user/', null=True, blank=True)
    # 用户姓名
    uname = models.CharField(u'用户昵称', max_length=15, null=True, blank=True)
    # 用户邮箱
    ueaddress = models.CharField(u'用户邮箱', max_length=50, null=True, blank=True)
    # 用户密码
    upassword = models.CharField(u'用户密码', max_length=16, null=True, blank=True)
    # 用户上次登录时间
    uLastLoginTime = models.DateTimeField(
        u'用户上次登录时间', auto_now=True, null=True, blank=True)
    # 用户生日
    uBrithday = models.DateTimeField(u'用户生日时间', null=True, blank=True)
    # 用户购物车
    # ucart
    uVistor = models.BooleanField('是否为游客', default=False)
...
class recentview(models.Model):
    ...
    # 浏览时间
    vtime = models.DateTimeField(u'浏览时间', auto_now=True)
